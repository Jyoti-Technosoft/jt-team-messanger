...
function ShowUsersChat() {
  ...
  const { setShowChat, userArr, loggedUserId, token, currentUserObj, setCurrentUserObj } = useContext(ShowContext);
  ...
  const setEmojiInChat = (value) => {
    userMsgInputFormik.setValues({ message: userMsgInputFormik.values.message + value.emoji });
  }
            ...
            <div className="header-chat-box">
              <div className="user-details">
                <div className="other-user-icon d-flex justify-content-center align-items-center">
                  <h5 className="first-char-text mb-0">{`${CapitalizeFirstLetter(currentUserObj?.firstName)}${CapitalizeFirstLetter(currentUserObj?.lastName)}`}</h5>
                </div>
                <p className="other-user-name">
                  {currentUserObj?.firstName}{" "}
                  {currentUserObj?.lastName}
                </p>
              ...
              <div className="message-div position-relative">
                <form onSubmit={userMsgInputFormik.handleSubmit}>
                  <input
                    ...
                    autoComplete="off"
                  />
                  <button className="send-btn position-absolute border-0" type="submit">
                    <img
